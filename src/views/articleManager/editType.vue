...
<template>
    <div class="editType tui-wh100">
        ...
        <el-form ref="form" label-width="6em">
            <el-form-item label="文章类型" class="is-required">
                <el-input v-model="form.name"   maxlength="50" show-word-limit v-validate
                          data-rules="required" validate-name="name" validate-type="keyup"
                          validate-tips-required="请输入文章类型"></el-input>
                <el-alert v-if="errors.get('name')!=null" :title="errors.get('name')" type="error"/>
            </el-form-item>
            <el-form-item label="上级类型" class="wid50">
            ...
            <el-form-item label="描述">
               <el-input type="textarea" :autosize="{ minRows: 2}"  v-model="form.description" maxlength="500" show-word-limit ></el-input>
            </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
        ...
        methods: {
            ...
            onSubmit() {
                if(this.$validator.checkAll()){
                    ...
                    this.$http.post("article/articleType/edit", editData).then(res => {
