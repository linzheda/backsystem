<template>
    <div class="test">
        <table-transfer ref="eltTransfer"
                        :show-query="true"
            ...
            <template v-slot:default="{scope}">
                <div>
                    <span v-if="scope.col.prop === 'gender'">{{scope.row.gender === '男' ? '男生' : '女生'}}</span>
                    <span v-else>{{scope.row[scope.col.prop]}}</span>
                </div>
            </template>
...
<script>
    ...
    export default {
        ...
        created() {
            setTimeout(()=>{
                this.tableData=[  {name: '方琼岚', gender: '女', age: 15},
        ...
        methods: {
            paginationCallBack(obj) {
                ...
                return new Promise(((resolve, reject) => {
                    try {
                        resolve({total: this.data1.length, data: this.data1})
                    } catch {
                        reject()
                    }
                }))
