<template>
    <div class="dictManager">
        ...
        <el-dialog v-el-drag-dialog
                   ...
                   width="40%">
            <edit-dict :showDialog.sync=showEditDialog @reloadData="loadData($event)"
                       :editData="editData"></edit-dict>
        </el-dialog>
    </div>
</template>
        ...
        methods: {
            ...
            handleDelete(data) {
                this.$confirm('您确定要删除这条记录吗?', '提示', {
                    ...
                    this.$http.post('dictionary/dictionary/delDict', {id: data['id']}).then(res => {
                        if (res['data']) {
                            this.$message({
